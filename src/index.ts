...
function readMore(buttons: NodeListOf<HTMLButtonElement>) {
	buttons.forEach((button) => {
		...
		const items =
			button.parentElement?.querySelector<HTMLUListElement>(listItems);

		button.addEventListener('click', () => {
			items?.scrollTo({
				top: items.scrollTop + parseInt(goTo),
				behavior: 'smooth',
			});
		});
	});
...
function setupHorizontalScroll() {
	const articles =
		document.querySelector('[aria-label="Articles and posts"]') ??
		document.querySelector('Artigos e postagens');
	document.addEventListener(
		...
		(e) => {
			...
			if (
				!['DL', 'DT', 'DD'].includes(target.tagName) &&
				!articles?.contains(target)
			)
				return;
			...
			if (['DT', 'DD'].includes(target.tagName)) target = target.parentElement!;
...
function onloaded() {
	const readMoreButtons = document.querySelectorAll<HTMLButtonElement>(
		'button[data-list-items]'
	);
